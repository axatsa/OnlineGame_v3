...
- [ ] Только один способ переводов (`i18next`), `LangContext` удалён
- [ ] `ErrorBoundary` на всех генераторах, нет белых экранов
- [ ] При перезагрузке страницы `activeClass` восстанавливается < 500мс
- [ ] Минимум 6 тестов из списка выше — зелёные
- [ ] Sentry настроен и получает ошибки в prod
- [ ] Все API-пути используют `/api/v1/`
